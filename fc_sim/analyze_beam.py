...
# beam may be a list of solids (multiple volumes); take the largest one by volume
if isinstance(beam, list):
    beam = max(beam, key=lambda s: s.Volume)
...
# Find bracket attach faces (connects to seat bracket at z≈30-36, x≈-30..29, y≈25.5..30.5)
# Find chassis attach faces (connects to chassis at x≈-33..-29 or 29..33)
beam_faces_bracket = []
beam_faces_chassis = []

for i, face in enumerate(obj.Shape.Faces):
    c = face.CenterOfMass
    # Bracket end: higher z (30-36), y around 28
    if c.z > 30 and 25 < c.y < 31:
        beam_faces_bracket.append("Face%d" % (i+1))
    # Chassis end: lower z (30-36), x near ±33
    if c.z > 30 and (c.x < -30 or c.x > 30):
        beam_faces_chassis.append("Face%d" % (i+1))

print("Beam bracket faces:", beam_faces_bracket)
print("Beam chassis faces:", beam_faces_chassis)

# Load at bracket end, fixed at chassis end (981 N)
if beam_faces_bracket and beam_faces_chassis:
    vm, disp, sf = run_static(obj, beam_faces_chassis, beam_faces_bracket, 981.0, "beam_bracket_to_chassis", 6.0)
    if vm:
        log_result("beam_bracket_to_chassis", 981, vm, disp, sf, "981N at bracket end, fixed at chassis end")
        print("beam: VM=%.1f MPa, disp=%.4f mm, SF=%.2f" % (vm, disp, sf))

# Also reverse: load at chassis, fixed at bracket (suspension compression)
if beam_faces_bracket and beam_faces_chassis:
    vm, disp, sf = run_static(obj, beam_faces_bracket, beam_faces_chassis, 981.0, "beam_chassis_to_bracket", 6.0)
    if vm:
        log_result("beam_chassis_to_bracket", 981, vm, disp, sf, "981N at chassis end, fixed at bracket end")
        print("beam rev: VM=%.1f MPa, disp=%.4f mm, SF=%.2f" % (vm, disp, sf))