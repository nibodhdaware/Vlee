"""Probe: kinematic pitch control to STAIR_ANGLE + sprocket drive + high friction."""
import math
...
def run():
    ...
    while t < 30.0:
        ...
        if not lifting and pos[0] > 0.85 and abs(lin[0]) < 0.15:
            lifting = True
            print(f"  kinematic pitch: handoff at x={pos[0]*100:.1f}cm, pitch={math.degrees(pitch):+.1f}")

        if lifting:
            # Ramp pitch to STAIR_ANGLE over 2 seconds
            # (In practice, we'd detect when near target and hold)
            # For now, just snap to target after handoff
    ...
    print(f"kinematic pitch: final x = {rows[-1][1]:.1f} cm, max_pitch = {max(r[3] for r in rows):+.1f}")
    p.disconnect()
